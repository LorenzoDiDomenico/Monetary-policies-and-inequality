# Modello AB-SFC -Monetary policies and distribution
...
eterogeneita=0# se =1 c'è eterogeneità nelle aspettative 
sceltastocastica=1#se 1 la scelta dell'impresa da cui consumare ? stocastica
sceltastocasticaK=1 # se 1 la scelta del fornitore da cui comprare il bene capitale ? stocastica
equaldistribution=0 #network iniziale household - imprese di consumo
illusion=1 # se =1 allora le famiglie utilizzano il valore di mercato della ricchezza nella funzione del consumo
householdcompranobond=1
householdcompranoequity=1
heterogenity_bond=1
heterogenity_equity=1
if(householdcompranobond==1){
  perc_agg=0.3 #percentuale aggregarta di titoli pubblici sul totale
}else{
  perc_agg=0 
}

if(householdcompranoequity==1){
  perc_agg_equity=0.3# #percentuale aggregata equity
}else{
  perc_agg_equity=0
  
}
per_V_det_da_int=-2 # se 0 le de+cisioni di portafoglio non dipendono da tosso d'interesse sui titoli e rendimento atteso equity
...
for(uu in 1:nsim_uu){
  ...
  creditcrunchoverGDP_stat=matrix(data=0, ncol=T,nrow=nsim)
          ...
          for(ll in 1:Ndk){
            ...
            for(xx in 1:Nperc){
              ...
              for(oo in 1:Nqex){
                ...
                betavalue=0.2 #95   #aspettativa adattiva
  ...
  for(jj in 1:nsim){
    ...
    espe=0# se lo metto uguale a zero assumo che la quantita che mi attendo per domani ? indipendente dal tasso di crescita atteso.. ma viene calcolato solo utilizzando direttametne le quantit? registrate nei periodi precedente (non la variazione dei tassi di crescita)
    conta=1
    xvalue=parametri[trial,20] #produttivit? del bene capitale (sarebbe: 1/v dell  a tesi)
    ...
    sens=0 #0.005  # sensibilit? del salario nominale rispetto al rapporto tra disoccupazione effettiva e full employment 
    ...
    t1=0.1 # sensibilit? rispetto a interesse target
    t2=0.9 # sensibilit? rispetto al tasso d'interesse realizzato nel periodo precedente 
    ...
    de=dk
    #N=3500 #max(2000,round((qexogenousc*NFC/(xvalue*v)+(((qexogenousc*NFC/xvalue)/z)/dk)/phi)),3)
    
    flexsigma=parametri[trial,11]
    sigmavalue=0.07 #0.05 #0.02 #& scorte desiderate
    perc=parametri[trial,7]
    cworker=0.6 #0.9 #0.85 #propensione al consumo dei lavoratori
    ...
    bankriskaversion=3.9
    creditcrunchtime=200
    ...
    for (mc in 1:MC) {
      ...
      if(TRUE){
        ...
        OCF=matrix(data=0,nrow=T,ncol=NF)
        OCFaverage=matrix(data=0,nrow=T,ncol=NF)
        ...
        loandemand_desired=matrix(data=0,nrow=T,ncol=NF)
        loandemand_approved=matrix(data=0,nrow=T,ncol=NF)
        ...
        ammortamento_loan=matrix(data=0,nrow=T,ncol=NF)
        amm_equity=matrix(data=0,nrow=T,ncol=NF)
        ...
        #vendite reali
        realsales=matrix(data=0,nrow=T+dk, ncol=NF)
        #costi correnti
        ...
        Iinstallednominal_int=matrix(data=0, nrow=T,ncol=NF)
        ...
        kdordinatofeasible=matrix(data=0, nrow=T+dk, ncol=NF)
        ...
        cashavailablefundingw=matrix(data=0,nrow=T, ncol=NF)
        cashavailablefundingk=matrix(data=0,nrow=T, ncol=NF)
        ...
        stockperditaB=matrix(data=0, nrow=T)
        ...
        loancrunched=matrix(data=0, nrow=T)
        GDP=matrix(data=0,nrow=T)
        creditcrunchoverGDP=matrix(data=0,nrow=T)
      ...
      for(i in 1:dk){
        for(j in (NFK+1):NF){
          ...
          Iinstallednominal_int[i,j]=Installarenominale[i]
          ...
          storicoI[i+z,j]=Iinstalled[i,j]
          #storicoIN[i+z,j]=Iinstallednominal_int[i,j]
          storicoIN[i+z,j]=Installarenominale[i]*(1+spam2*exintrate*lev_net)
          storicoIN_noint[i+z,j]=Installarenominale[i]
        }
      }
      tol <- 1e-7
      
      loan_value <- function(L) {
        # Calcola TotalexpectedCF dato un importo di prestito L
        newdebtservice <- L * (1 + spam2 * intrate[t]) / z              # rata debito per periodo
        pr_L <- 1 / (1 + exp((OCFfutures - bankriskaversion * newdebtservice) / newdebtservice))
        # Serie residuo capitale da rimborsare a inizio di ciascun periodo (come in modello)
        residuo_inizio_L <- L * ((z - h) * (z - h + 1)) / (z * (z + 1))  # h = 0:(z-1)
        # Flussi attesi totali condizionali (somma su tutti i periodi)
        ExpectedCF_L <- sum( (1 - pr_L)^(0:(z-1)) * ( (1 - pr_L) * newdebtservice 
                                                      + pr_L * pmin(k[t-1, i] * priceindexK[t-1], residuo_inizio_L) ) )
        return( -L + ExpectedCF_L )   # TotalexpectedCF(L)
      }
      ...
      for (t in 1:T){
        ...
        # Fixing lambdas in households portfolio preferences #####
        #Fixing lambda to have that in the heterogeonus shenarios the aggregate percentages of each assets on total wealth are the same of the homogenous scenario preferences 
        if(t==100){
        ...
        if(t>1){
          cashF[t,]=cashF[t-1,]
          inv[t,1:NFK]=inv[t-1,1:NFK]
        }
        
        if(length(which(cashF[t,(NFK+1):NF]<0))>0){
          print("cashF negativo")
          stop()
        }
        ...
        #Expectations on macroeconomic variables (not used in the current verision of the model)        
        if(t>4){
        ...
        #Check loan to finance investments oand credit crunch ##############
        for(i in (NFK+1):NF){
          if(t>dk){
            cashavailablefundingk[t,i]=min(cashF[t,i],retained_profit[t-1,i]+amm_equity[t-1,i]+max(E_received[t-1,i],0))
            cashavailablefundingk[t,i] = max(0, cashavailablefundingk[t,i])  #This is redundant
            loandemand_desired[t,i]=max(kdfeasiblenominal[t,i]*lev_net,kdfeasiblenominal[t,i]-cashavailablefundingk[t,i])
            
            if(FALSE){
              # Calcolo degli OCF in ogni periodo futuro considerando come variabile la parte di servizio al debito (man mano che passa il tempo, alcuni prestiti vanno a maturità)
              
              pr=matrix(data=0, nrow=z)
              OCFfutures=matrix(data=0, nrow=z)
              SDfutures=matrix(data=0, nrow=z)
              newdebtservice=matrix(data=0, nrow=z)
              ammresiduofutures=matrix(data=0, nrow=z)
              
              TotalLoanint <-  loandemand_desired[t,i] * (1 + spam2 * intrate[t])
              spreadLoan <- ((z:1)/z) / spam
              newdebtservice  <- TotalLoanint * spreadLoan   
              h <- 0:(z-1)  
              residuo_inizio <-  loandemand_desired[t,i]* ((z - h) * (z - h + 1)) / (z * (z + 1))  # prima del pagamento, infatti se fallisce non mi rida il rimborso di quel periodo
              ...
              for(er in 1:z){
                ...
                for(j in (t+er):(t+z-1)){
                  if(loanstoricoF[j,i]>0){
                    SDfutures[er]=SDfutures[er]+loanstoricoFtot[j,i]*g/spam
                  }
                  g=g+1/z
                }  
                OCFfutures[er]=OCFaverage[t-1,i]    #revenues[t-1,i]-wb[t-1,i]*(1+leveragew[t-1,i]*spamshort*intrateshort[t-1])-SDfutures[er]
                pr[er]=1/(1+e^((OCFfutures[er]-bankriskaversion*newdebtservice[er])/newdebtservice[er]))
                ExpectedCF[er]=cumprob*((1-pr[er])*newdebtservice[er] + pr[er]*min(kfutures[er]*priceindexK,residuo_inizio[er]))
                cumprob=cumprob*(1-pr[er])
              }
              TotalexpectedCF=-loandemand_desired[t,i]+sum(ExpectedCF)
            }
            
            #Credit crunch evaluation 
            
            ExpectedCF=matrix(data=0, nrow=z)
            SDexpected=0
            g=1/z
            for(j in (t+1):(t+z)){
              if(loanstoricoF[j,i]>0){
                SDexpected=SDexpected+loanstoricoFtot[j,i]*g/spam
              }
              g=g+1/z
            }
            
            newdebtservice=loandemand_desired[t,i]*(1+spam2*intrate[t])/z
            #OCFfutures=OCFaverage[t-1,i]  
            OCFfutures=revenues[t-1,i]-wb[t-1,i]*(1+leveragew[t-1,i]*spamshort*intrateshort[t-1])-SDexpected
            pr=1/(1+e^((OCFfutures-bankriskaversion*newdebtservice)/newdebtservice))
            
            h <- 0:(z-1)  
            residuo_inizio <- loandemand_desired[t,i] * ((z - h) * (z - h + 1)) / (z * (z + 1))  # prima del pagamento, infatti se fallisce non mi rida il rimborso di quel periodo
            for(er in 1:z){
              ExpectedCF[er]=(1-pr)^(er-1)*((1-pr)*newdebtservice + pr*min(k[t-1,i]*priceindexK[t-1],residuo_inizio[er]))
            }
            TotalexpectedCF=-loandemand_desired[t,i]+sum(ExpectedCF)
            
            
            
            if(TotalexpectedCF<0 & t>creditcrunchtime){
              n_credit_crunch=n_credit_crunch+1
              
              
              
              # Determinazione dell'importo massimo L_opt che rende TotalexpectedCF ~ 0
              L_lower <- 0
              L_upper <- loandemand_desired[t, i]
              L_min_pos <- max(1e-6, 1e-6 * L_upper)
              
              # Valuta segni agli estremi
              val_lower <- loan_value(L_min_pos)
              val_upper <- loan_value(L_upper)
              if(val_upper >= 0) {
                # Caso limite: anche l'intero prestito è sostenibile (nessun credit crunch effettivo)
                L_opt <- L_upper
              } else if(val_lower < 0) {
                # Caso opposto estremo: anche un prestito piccolissimo dà valore atteso negativo
                L_opt <- 0
              } else {
                # Ricerca della radice nell'intervallo [L_min_pos, L_upper]
                L_opt <- uniroot(function(L) loan_value(L),
                                 lower = L_min_pos, upper = L_upper,
                                 tol = 1e-8)$root
              }
              
              if(L_opt<0 | L_opt>loandemand_desired[t,i]){
                print("problem laond demand crunch")
                stop()
              }
              
              loandemand_approved[t, i] <- L_opt
              #loandemand_approved[t,i]=0
              loancrunched[t]=loancrunched[t]+loandemand_desired[t,i]
              
              
              kdfeasiblenominalnew=min(loandemand_approved[t,i]+cashavailablefundingk[t,i],kdfeasiblenominal[t,i])
              inv[t,indexFK[t-dk,i]]=inv[t,indexFK[t-dk,i]]+(kdordinatofeasible[t,i]-kdfeasiblenominalnew/p[t-dk,indexFK[t-dk,i]])
              
              realsales[t,indexFK[t-dk,i]]= realsales[t,indexFK[t-dk,i]]-(kdordinatofeasible[t,i]-kdfeasiblenominalnew/p[t-dk,indexFK[t-dk,i]])
              revenues[t, indexFK[t-dk,i]]= realsales[t,indexFK[t-dk,i]]*p[t-dk,indexFK[t-dk,i]] 
              #  storicovaluesails[t, indexFK[t-dk,i]]=
              # storicosales[t, indexFK[t-dk,i]]=
              kdfeasiblenominal[t,i]=min(loandemand_approved[t,i]+cashavailablefundingk[t,i],kdfeasiblenominal[t,i])
              kdordinatofeasible[t,i]=kdfeasiblenominal[t,i]/p[t-dk,indexFK[t-dk,i]]
              
            }else{
              loandemand_approved[t,i]=loandemand_desired[t,i]
              #lascio invariati kdfeasiblenominal e kdordinatofeasible
            }
            loandemandF[t,i]=loandemand_approved[t,i]
            
            if(kdfeasiblenominal[t,i]>0){
              leverage_gross[t,i]=loandemandF[t,i]/kdfeasiblenominal[t,i]
            ...
            if(leverage_gross[t,i]>1){
              print("probelm leverage")
              stop()
            }
            ...
            #devo prima definire il leverage
            Iinstalled[t,i]=kdordinatofeasible[t,i]
            Iinstallednominal_int[t,i]=kdfeasiblenominal[t,i]*(1+spam2*intrate[t]*leverage_gross[t,i])
            Iinstallednominal_noint[t,i]=kdfeasiblenominal[t,i]
            
            #kdordinatofeasiblestorico[t+dk-1,i]=kdordinatofeasible[t,i]
            kdordinatofeasiblenominalstorico_noint[t+dk-1,i]=kdfeasiblenominal[t,i]
            #kdordinatofeasiblenominalstorico[t+dk-1,i]=kdfeasiblenominal[t,i]*(1+spam2*intrate[t]*leverage_gross[t,i])
            kdordinatofeasiblenominalstorico_loan[t+dk-1,i]=kdfeasiblenominal[t,i]*leverage_gross[t,i]*(1+spam2*intrate[t]) # è uguale al precedente
            kfutinstorico_nominal_interest[t+z,i]=kdfeasiblenominal[t,i]*(1+spam2*intrate[t]*leverage_gross[t,i])
            kfutinstorico[t+z,i]=kdordinatofeasible[t,i]
            kfutinstorico_nominal[t+z,i]=kdfeasiblenominal[t,i]
            storicoI[t+z,i]=Iinstalled[t,i]
            storicoIN[t+z,i]=Iinstallednominal_int[t,i]   #costo effettivo del capitale con interesssi effettiv
            storicoIN_noint[t+z,i]=Iinstallednominal_noint[t,i]   #costo del capitale senza interessi
            storicoIN_loan[t+z,i]=kdfeasiblenominal[t,i]*leverage_gross[t,i]*(1+spam2*intrate[t])    #quota parte di capitale pagate con debito e annessi interssi
          }
          
          if(is.nan(leverage_gross[t,i]) || is.infinite(leverage_gross[t,i])){
            print("problem leverage")
            stop()
            #leverage_gross[t,i]=0
          }
        ...
        #aggiornamento capitale:devo calcolare il deprezzamento per ogni singolo capitale installato precedentemente che ad oggi compone tutto il capitale che dispongo
        ...
        for(j in (t+1):(t+z)){
          ...
          ammortamento_loan[t,(NFK+1):NF]=ammortamento_loan[t,(NFK+1):NF]+storicoIN_loan[j,(NFK+1):NF]*g/spam
          #ammortamentofull[t,(NFK+1):NF]=ammortamentofull[t,(NFK+1):NF]+storicoINfull[j,(NFK+1):NF]*g/spam
          amm_equity[t,(NFK+1):NF] = amm_equity[t,(NFK+1):NF] + storicoIN_noint[j,(NFK+1):NF] * g/spam * lev_equitystorico[j,(NFK+1):NF]
        ...
        if(t>1){
          kvalue[t,(NFK+1):NF]=kvalue[t-1,(NFK+1):NF]+Iinstallednominal_int[t,(NFK+1):NF]-ammortamento[t,(NFK+1):NF]
        }else{
          kvalue[t,(NFK+1):NF]=Iinstallednominal_int[t,(NFK+1):NF]-ammortamento[t,(NFK+1):NF]
        ...
        if(TRUE){
          if(t>2){
            q_e[t,which(cod==1 & bancarotta[t-1,]==0)]=ifelse(round(inv[t-1,which(cod==1 & bancarotta[t-1,]==0)],5)>0,beta[which(cod==1 & bancarotta[t-1,]==0)]*realsales[t-1,which(cod==1 & bancarotta[t-1,]==0)]+(1-beta[which(cod==1 & bancarotta[t-1,]==0)])*q_e[t-1,which(cod==1 & bancarotta[t-1,]==0)],(beta[which(cod==1 & bancarotta[t-1,]==0)]*realsales[t-1,which(cod==1 & bancarotta[t-1,]==0)]+(1-beta[which(cod==1 & bancarotta[t-1,]==0)])*q_e[t-1,which(cod==1 & bancarotta[t-1,]==0)])*1)  #nel caso di memoria storica con pesi decrescenti 
            if(length(which(bancarotta[t-1,]==1))>0){
              stocvec=abs(rfoldnorm(length(which(bancarotta[t-1,]==1)),mean=0.1,sd=0.0096))
              q_e[t,which(cod==1 & bancarotta[t-1,]==1)]=stocvec*(beta[which(cod==1 & bancarotta[t-1,]==1)]*realsales[t-1,which(cod==1 & bancarotta[t-1,]==1)]+(1-beta[which(cod==1 & bancarotta[t-1,]==1)])*q_e[t-1,which(cod==1 & bancarotta[t-1,]==1)])
            ...
            cap_depreciated_value_int[t,(NFK+1):NF]=kvalue_used[t,(NFK+1):NF]-capfuturo_value[t,(NFK+1):NF]
            ...
            if(t==1){
              ...
              q_e[t,(NFK+1):NF]=realsales[t-1,(NFK+1):NF] #*b+demandperfirmconsumer[t-2,i]*(1-b): dal terzo periodo#posso cambiarla e metterci quella in toward a benchmark model stigliz ecc
            }
            q_dn[t,(NFK+1):NF]=q_e[t,(NFK+1):NF]*(1+espe*gex[t,(NFK+1):NF])#*(1+sigma)
            k_de[t,(NFK+1):NF]=q_dn[t,(NFK+1):NF]/(u_n*x[(NFK+1):NF])
            szz=1/z
            for(j in (t+dk+1):(t+z+dk)){
              capfuturo[t,(NFK+1):NF]=capfuturo[t,(NFK+1):NF]+kfutinstorico[j,(NFK+1):NF]*szz
              capfuturo_value[t,(NFK+1):NF]=capfuturo_value[t,(NFK+1):NF]+kfutinstorico_nominal_interest[j,(NFK+1):NF]*szz
              capfuturo_value_noint[t,(NFK+1):NF]=capfuturo_value_noint[t,(NFK+1):NF]+kfutinstorico_nominal[j,(NFK+1):NF]*szz
        ...
        if(TRUE){
          ...
          if(dk>1){
            ...
            y_d[t,1:NFK]=demandperfirmcapital[t,1:NFK]-inv[t,1:NFK]
          }
        ...
        if(TRUE){
          for(i in 1:NFK){
            ...
            if(t==1){
              ...
              inv[t,i]=y_new[t,i]+inv[t,i]
            }
            #realsales[t,i]=storicoordini[t,i]*dk
            #inv[t,i]= inv[t,i]-realsales[t,i]
            # revenues[t,i]=realsales[t,i]*storicoprezzo[t,i]
            ...
            if(length(customC)>0){
              ...
              for(j in 1:length(customC)){
                ...
                if(isTRUE(all.equal(kd[t,indexCK]/dk,inv[t,i],tolerance=tol)) || (kd[t,indexCK]/dk)<inv[t,i]){
                  ...
                  kdordinatofeasible[t+dk,indexCK]=kd[t,indexCK]
                  ...
                  kdordinatofeasible[t+dk,indexCK]=inv[t,i]*dk
        ...
        #kdfeasiblenominal_net2[t+dk,(NFK+1):NF]=(kdfeasible[t+dk,(NFK+1):NF]-cap_depreciated[t+dk,(NFK+1):NF])*p[t,indexFK[t-1,(NFK+1):NF]]
        #kdfeasiblenominal_net2[t+dk,(NFK+1):NF]=kdfeasiblenominal[t+dk,(NFK+1):NF]-cap_depreciated_value_int[t,(NFK+1):NF]
        ...
        #  stop()
        #}
        realsales[t+dk, 1:NFK]=sails[t, 1:NFK]*dk
        ...
        for(i in (NFK+1):NF){
          cashavailablefundingw[t,i]=0 
          if(t>1){
            shortloandemandF[t,i]=max(wb[t,i]*lev_w,wb[t,i]-cashavailablefundingw[t,i])
            if(wb[t,i]>0){
              leveragew[t,i]=shortloandemandF[t,i]/wb[t,i]
            }else{
              leveragew[t,i]=0  
            }
            Esupply[t,i]=(1-lev_net)*kdfeasiblenominal_net2[t+dk,i]   #kdfeasiblenominal_net[t+dk,i]
          }else{
            shortloandemandF[t,i]=wb[t,i]
            if(wb[t,i]>0){
              leveragew[t,i]=shortloandemandF[t,i]/wb[t,i]
            }else{
              leveragew[t,i]=0  
          ...
          if(intrate[t]==0){
            ...
            if(is.nan(storicoratashort[t+zs,i])==TRUE){
        ...
        if(FALSE){  
          if(t>1){
            ...
            for(i in 1:NFK){
              ...
              }else if(limalzare[t-1,i]>limiteinvinc){      #(inv[t-1,i]<invt[t-1,i]  | demandperfirmcapital[t-1,i]>realsales[t-1,i]){
                #stocnum=sample(seq(0.001,0.005,0.001),1)
                stocnum=abs(rfoldnorm(1,mean=mediaran,sd=sdran)-1)
                markup[t,i]=max(markupmin,markup[t-1,i]*(1+stocnum))  
                p[t,i]=unitcost[t,i]*(1+markup[t,i])
        ...
        if(TRUE){  
          for(i in (NFK+1):NF){
            realsales[t,i]=min(inv[t,i],(G[t]*alfak[t,i])/p[t,i])
            inv[t,i]=inv[t,i]-realsales[t,i]
            if(inv[t,i]==0){
              Gef[t]=Gef[t]+realsales[t,i]*p[t,i]
        ...
        Gcons[t]=Gef[t]
        Greal[t]=sum(realsales[t,which(cod==1)])
              ...
              while(q!=1){
                ...
                if(length(availablesuppliers)>0){
                  ...
                  if(realconsumptiondemand[t,indexH]<=inv[t,indexC[t,indexH]]){
                    realsales[t,indexC[t,indexH]]=realsales[t,indexC[t,indexH]]+realconsumptiondemand[t,indexH]
                    ...
                    realsales[t,indexC[t,indexH]]=realsales[t,indexC[t,indexH]]+inv[t,indexC[t,indexH]]
        ...
        revenues[t,(NFK+1):NF]=realsales[t,(NFK+1):NF]*p[t,(NFK+1):NF]
        
        unsatisfieddemand[t]=sum(realconsumptionmissing[t,]) #Greal[t]+sum(realconsumptiondemand[t,])-sum(realsales[t,which(cod==1)])
        ...
        # if(t==2){
        #   demandperfirmconsumer_sign[t,(NFK+1):NF]=realsales[t,(NFK+1):NF]*beta+(1-beta)*realsales[t-1,(NFK+1):NF]
        ...
        #   
        # }else if(t>2){
        #   demandperfirmconsumer_sign[t,(NFK+1):NF]=realsales[t,(NFK+1):NF]*beta+(1-beta)*demandperfirmconsumer_sign[t-1,(NFK+1):NF]
        ...
        # }else{
        #   demandperfirmconsumer_sign[t,(NFK+1):NF]=realsales[t,(NFK+1):NF]
        ...
        if(TRUE){ 
          for(i in (NFK+1):NF){
            ...
            if(is.nan(inv[t,i]/realsales[t,i])==TRUE){
              invlimsup[t,i]=0 #invlimsup[t-1,i]
              invliminf[t,i]=0 #invlimsup[t-1,i]
            }else{
              if((inv[t,i]/realsales[t,i])<sigma*(1-flexsigma)){
                if(t==1){
                  invlimsup[t,i]=1
                  invliminf[t,i]=0 
                }else{  
          ...
          if(t>1 && equities_tot[t-1]>0){
            if(E_demand_tot[t]>=E_supply_tot[t]){
              ...
              if(p_e[t]>0){
                ...
                print("problem negative equity price or zero")
                stop()
              }
        ...
        # Pagamento deel servizio al dwbito ##############
        for(i in (NFK+1):NF){
          ...
          cashF[t,i]=cashF[t,i]+loandemandF[t,i]+E_received[t,i]+shortloandemandF[t,i]+revenues[t,i]-wb[t,i]-kdfeasiblenominal[t,i]-servicedebtFtot[t,i]  #-ponzis[(t-1),i]                                                                                                         
          if(t>1){
          ...
          if(cashF[t,i]>=0){
            ...
            print("entering in the negative CF zone")
            print(t)
            ExpectedCF=matrix(data=0, nrow=z)
            SDexpected=0
            g=1/z
            ...
            residuo_inizio <- loandemand_desired[t,i] * ((z - h) * (z - h + 1)) / (z * (z + 1))  # prima del pagamento, infatti se fallisce non mi rida il rimborso di quel periodo
            for(er in 1:z){
              ExpectedCF[er]=(1-pr)^(er-1)*((1-pr)*newdebtservice + pr*min(k[t,i]*priceindexK[t-1],residuo_inizio[er]))
            }
            TotalexpectedCF=-loandemand_desired[t,i]+sum(ExpectedCF)
            
            #NW[t,i]=kvalue2_noint[t,i]+(cashF[t,i]+servicedebtFtot[t,i])-stockdebtF[t,i]+inv[t,i]*unitcost[t,i]
            #Residualvalue[t,i]=kvalue2_noint[t,i]+inv[t,i]*unitcost[t,i]
            if(TotalexpectedCF>0  | t<creditcrunchtime){
              ...
              interestpaymentF[t,i]=max(0, interestpaymentF[t,i]-max(0,abs(cashF[t,i])-servicedebtF[t,i]))
              servicedebtF[t,i]=max(0,servicedebtF[t,i]-abs(cashF[t,i]))
        ...
        # 
        if(t>1){
          inv_cont[t,1:NFK]=inv_cont[t-1,1:NFK]+y[t,1:NFK]-realsales[t,1:NFK]
        }else{
          inv_cont[t,1:NFK]=y[t,1:NFK]-realsales[t,1:NFK]
        }
        valueinv[t,1:NFK]=inv_cont[t,1:NFK]*wageaggregate[t]/phi
        if(t>1){
          for(i in (NFK+1):NF){
            if(inv[t-1,i]>=realsales[t,i]){
              valueinv[t,i]=(valueinv[t-1,i]/inv[t-1,i])*(inv[t-1,i]-realsales[t,i])+y[t,i]*unitcost[t,i]
              valueinv2[t,i]=(valueinv2[t-1,i]/inv[t-1,i])*(inv[t-1,i]-realsales[t,i])+y[t,i]*wageaggregate[t]/(xexogenous*v)
            }else{
        ...
        currentcost_profit[(NFK+1):NF]=ammortamento_loan[t,(NFK+1):NF]+amm_equity[t,(NFK+1):NF]+wb[t,(NFK+1):NF]*(1+intrateshort[t]*leveragew[t,(NFK+1):NF]*spamshort)
        ...
        OCF[t,(NFK+1):NF]=revenues[t,(NFK+1):NF]-wb[t,(NFK+1):NF]-servicedebtFtot[t,(NFK+1):NF]
        
        
        if(t>1){
          OCFaverage[t,(NFK+1):NF]=OCFaverage[t-1,(NFK+1):NF]+alfa[(NFK+1):NF]*(OCF[t-1,(NFK+1):NF]-OCFaverage[t-1,(NFK+1):NF])
        }
        
        
        profit[t,1:NFK]=pmax(0,revenues[t,1:NFK]-currentcost[1:NFK]) # ci andrebbero messe le inventories
        ...
        NW[t,(NFK+1):NF]=kvalue2_noint[t,(NFK+1):NF]+cashF[t,(NFK+1):NF]-stockdebtF[t,(NFK+1):NF]+valueinv[t,(NFK+1):NF]-equities_F[t,(NFK+1):NF]*p_e[t]# la ricchezza nettta in questo caso dovrebbe corrsiponde a profitti trattenuti pi? equity
        ...
        if(t>1){
          if(round(Deposit[t] - (StockLoanB[t] + creditconsumptionB[t]), 4)<0){
            ...
            Hb[t]=round(Hb[t-1]+deltaDeposit[t]-(LoanBDemand[t]+sum(creditdemand[t,])-sum(LoansHH[t-1,])-sum(capitalhh[t,])-sum(servicedebtF[t,])-sum(servicericapital[t,])-PerditaB[t]-deltaRes[t]),4)
          ...
          if(Deposit[t]<(StockLoanB[t]+nonperformingloanbank[t]+creditconsumptionB[t])){
            ...
            Hb[t]=deltaDeposit[t]-(LoanBDemand[t]+sum(creditdemand[t,])-sum(capitalhh[t,])-sum(servicedebtF[t,])-sum(servicericapital[t,])-PerditaB[t])
            
          }
          if(Deposit2[t]>(StockLoanB[t]+nonperformingloanbank[t]+creditconsumptionB[t])){
            Hb2[t]=deltaDeposit2[t]-(LoanBDemand[t]+sum(creditdemand[t,])-sum(capitalhh[t,])-sum(servicedebtF[t,])-sum(servicericapital[t,])-PerditaB[t])  #se i depositi attivi sono maggiori dei prestti, metto a riserva in modo da creare un entrata per pagare gli interessi (questa e' una tecnica rigidad nel senso che no nsfrutto il gioco sul differenziale dei tassi d interesse per coprirmi) 
          }else{
            Hb2[t]=0    
          }
        ...
        if(t>1){
          CFB[t]=CFB[t-1]+sum(servicedebtFtot[t,])-sum(servicedebtFtot[t,which(bancarotta[t,]==1)])-LoanBDemand[t]+deltaDeposit[t]+sum(servicericapital[t,])-sum(creditdemand[t,])+sum(effectivepaymentB[t,])
          stockperditaB[t]=stockperditaB[t-1]+PerditaB[t]
        }else{
          CFB[t]=sum(servicedebtFtot[t,])-LoanBDemand[t]+deltaDeposit[t]
          stockperditaB[t]=PerditaB[t]
        }
        #if(CFB[t]<0){
        # if(t>1){
        ...
        #Controlli
        redundant[t]=round(Deposit[t]+sum(EquityH[t,])-E_tot[t]-StockLoanB[t]-nonperformingloanbank[t]+stockperditaB[t]-creditconsumptionB[t]-BondCB[t]-Advances[t]+NWCB[t],4)
        redundant2[t]=round(Hb[t]-BondCB[t]-stockperditaB[t],3)
        ...
        totalrealsalesK=sum(realsales[t,which(cod==0)])
        
        
        
        # for(i in 1:NFK){
        # marketshare[t,i]=realsales[t,i]/totalrealsalesK   #vedere come cambia se metto la domanda ricevuta invece delle vendite
        # if(is.nan(marketshare[t,i])==TRUE){
        #   marketshare[t,i]=0
        #}
        # ratiomarketshare[t,i]=marketshare[t,i]/((1-realsales[t,i]/totalrealsalesK)/(NFK-1))
        ...
        #deb_yc[t,mc]=stockdebtG[t]/(sum(realsales[t,which(cod==1)]))
        ...
        totalrealsalesC=0
        totalrealsalesC=sum(realsales[t,which(cod==1)])
        
        # for(i in (NFK+1):NF){
        #   priceweightedC[i]=(realsales[t,i]/totalrealsalesC)*p[t,i]
        #   markupweightedC[i]=(realsales[t,i]/totalrealsalesC)*markup[t,i]
        # }
        # 
        if(totalrealsalesC>0){
          priceweightedC[(NFK+1):NF]=realsales[t,(NFK+1):NF]/totalrealsalesC*p[t,(NFK+1):NF]
          markupweightedC[(NFK+1):NF]=realsales[t,(NFK+1):NF]/totalrealsalesC*markup[t,(NFK+1):NF]
        }
        
        # for(i in 1:NFK){
        #   if(totalrealsalesK>0){
        #     priceweightedK[i]=(realsales[t,i]/totalrealsalesK)*p[t,i]
        #     markupweightedK[i]=(realsales[t,i]/totalrealsalesK)*markup[t,i]
        #   }
        # }
        bondfamiglietot[t]=sum(BondHH[t,])
        
        if(totalrealsalesK>0){
          priceweightedK[1:NFK]=(realsales[t,1:NFK]/totalrealsalesK)*p[t,1:NFK]
          markupweightedK[1:NFK]=(realsales[t,1:NFK]/totalrealsalesK)*markup[t,1:NFK]
        }
        ...
        creditcrunchoverGDP[t]=loancrunched[t]/GDP[t]
        ...
        if(TRUE){
          ...
          InvestmentR[t]=sum(kdordinatofeasible[t+dk,])
          ...
          if(t>1){
            gr[t,(NFK+1):NF]=(realsales[t,(NFK+1):NF]-realsales[t-1,(NFK+1):NF])/(realsales[t-1,(NFK+1):NF])
            ...
            gr[t,(NFK+1):NF]=ifelse(round(realsales[t-1,(NFK+1):NF],5)==0,1,gr[t,(NFK+1):NF])
        ...
        # if(t>1){
        #   gr[t,2]=(realsales[t,2]-realsales[t-1,2])/realsales[t-1,2]
        ...
        if(FALSE){  
          ...
          #FIXED CAPITAL
          BS[which(rownames(BS)=="Fixed Capital"),which(colnames(BS)=="FIRMSC"),t]=sum(kvalue2_noint[t,])
          #ADVANCES
          ...
          TM[which(rownames(TM)=="deltaloan"),which(colnames(TM)=="FIRMSC"),t]=sum(loandemandF[t,which(cod==1)])-sum(servicedebtF[t,which(cod==1)])+sum(shortloandemandF[t,which(cod==1)])+sum(overdraftdemand[t,which(cod==1)])-sum(servicericapital[t,which(cod==1)])-PerditaB[t] #quest' ultimo corrsiponde al debito cancellato
          TM[which(rownames(TM)=="deltaloan"),which(colnames(TM)=="BANKS"),t]=-(LoanBDemand[t]-sum(servicedebtF[t,])-sum(servicericapital[t,])-PerditaB[t])
    ...
    creditcrunchoverGDP_stat[jj,]=creditcrunchoverGDP
